...
`type` is required whenever `behavior` is present at all — enforced by
`TaskConfig.Validate` — and selects one of two mutually exclusive
resolution modes; the fields below only apply to the mode they belong to.

| Field          | Required                          | Purpose                                                                                                                                                                   |
|----------------|------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `type`         | yes, whenever `behavior` is set    | `"statusMap"` or `"autoApprove"`. Any other value (including empty/omitted) fails `TaskConfig.Validate`.                                                                  |
| `outcomeField` | no, `statusMap` mode only          | Key read from the `POST /api/v1/applications/{taskId}/review` request body. Defaults to `"review_outcome"` (`DefaultOutcomeField`) when empty.                            |
| `statusMap`    | no, `statusMap` mode only          | Maps the outcome field's value (e.g. `"approve"`) to the status stored on the application (e.g. `"APPROVED"`).                                                            |

Setting `outcomeField` or `statusMap` while `type` is `"autoApprove"` fails
`TaskConfig.Validate` — the two modes are mutually exclusive by construction,
not just by convention.

Resolution, on review submission:
1. If `type` is `"autoApprove"`, the status resolves unconditionally to
   `"APPROVED"` — no body field is read, no `statusMap` lookup happens. Use
   this for pure data-capture/confirmation/issuance tasks whose review form
   has no decision field at all (e.g. logging receipt of a physical sample,
   attaching an issued letter) — successfully submitting the form *is* the
   outcome.
2. If `type` is `"statusMap"`, read `body[outcomeField]` (or
   `body["review_outcome"]` if `outcomeField` is unset), then look that
   value up in `statusMap`.
...
Auto-approve example — no jsonform changes required, no synthetic decision
field:

```json
{
  "taskCode": "fcau_sample_wait_v1",
  "forms": {
    "view": "fcau-apply-health-cert--user-form",
    "review": "fcau-sample-collection--reviewer-form"
  },
